...
namespace PseudOfficial
{
    class OAuth
    {
        ...
        public void GetRequestToken(string consumerKey, string consumerSecret)
        {
            ...
            string signature = GenerateSignature("", "GET", URLs.REQUEST_TOKEN_URL, parameters);
            parameters.Add("oauth_signature", UrlEncode(signature));
            string response = HttpGet(URLs.REQUEST_TOKEN_URL, parameters);
            Dictionary<string, string> dic = ParseResponse(response);
            Settings.RequestToken = dic["oauth_token"];
            Settings.RequestTokenSecret = dic["oauth_token_secret"];
        ...
        {
            return URLs.AUTHORIZE_URL + "?oauth_token=" + Settings.RequestToken;
        }

        public void GetAccessToken(string pin)
        {
            SortedDictionary<string, string> parameters = GenerateParameters(Settings.RequestToken);
            parameters.Add("oauth_verifier", pin);
            string signature = GenerateSignature(
                Settings.RequestTokenSecret, "GET", URLs.ACCESS_TOKEN_URL, parameters);
            parameters.Add("oauth_signature", UrlEncode(signature));
            string response = HttpGet(URLs.ACCESS_TOKEN_URL, parameters);
            Dictionary<string, string> dic = ParseResponse(response);

            Settings.AccessToken = dic["oauth_token"];
            Settings.AccessTokenSecret = dic["oauth_token_secret"];
            Settings.UserID = dic["user_id"];
            Settings.ScreenName = dic["screen_name"];
        ...
        {
            SortedDictionary<string, string> parameters2 = GenerateParameters(Settings.AccessToken);
            foreach (var p in parameters)
                parameters2.Add(p.Key, p.Value);
            string signature = GenerateSignature(Settings.AccessTokenSecret, "GET", url, parameters2);
        ...
        public string Post(string url, IDictionary<string, string> parameters)
        {
            ...
            foreach (var p in parameters)
                parameters2.Add(p.Key, p.Value);
            string signature = GenerateSignature(Settings.AccessTokenSecret, "POST", url, parameters2);
            parameters2.Add("oauth_signature", UrlEncode(signature));
        ...
        private string GenerateSignature(string tokenSecret, string httpMethod, string url, SortedDictionary<string, string> parameters)
        {
            ...
            hmacsha1.Key = Encoding.ASCII.GetBytes(UrlEncode(Settings.ConsumerSecret) + '&' + UrlEncode(tokenSecret));
        ...
        private SortedDictionary<string, string> GenerateParameters(string token)
        {
            ...
            result.Add("oauth_consumer_key", Settings.ConsumerKey);
